document.addEventListener("DOMContentLoaded", function() {
  // Display the tooltip message for a few seconds on page load
  const tooltipMessage = d3.select(".tooltip-message");
  tooltipMessage.style("left", "50px")
      .style("top", "50px")
      .style("display", "block");
  ...
  d3.csv("data.csv").then(function(data) {
      ...
      // Filter out data points with Meta_score less than 40% and Gross earnings less than $100,000
      data = data.filter(function(d) {
          return d.Meta_score >= 40 && d.Gross >= 100000;
      });
      ...
      select.append("option").text("All Decades").attr("value", "all");
      ...
      const minYValue = 100000; // Minimum value set to $100,000
      const maxYValue = 1.5e9; // 1.5 billion

      // Set the x and y axis limits
      const xMin = 40; // Start X-axis at 40%
      const xMax = 100;
      ...
      const xAxisTickValues = [40, 50, 60, 70, 80, 90, 100];
      ...
      function updatePlot(selectedDecade) {
          ...
          if (selectedDecade !== "all") {
              filteredData = filteredDataAll.filter(d => d.Decade == selectedDecade);
          }
          // Clear old elements:
          ...
          svg.selectAll("circle")
              ...
              .attr("stroke", "#000") // Add a border to the circles
              .attr("stroke-width", 1)
